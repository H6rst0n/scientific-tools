...
### 3. 🧬 MO 能級圖繪製工具 (Molecular Orbital Energy Level Plotter)

> 為分子軌域、片段軌域與配位場相關圖設計的互動式出版級繪圖工具。

* **工具入口**：[`mo-level-plotter.html`](./mo-level-plotter.html)
* **主要功能**：
  * 任意新增、刪除、改名、隱藏與重排能級欄，內建 H₂O 單欄 MO 與 CO₂ 三欄相關圖範本。
  * 以一列管理一組簡併能級，分開設定軌域描述與群論對稱性（皆提供建議選項並允許自訂），逐軌域設定空軌域、向上／向下自旋或成對電子；電子箭頭可在能級邊界內用滑鼠水平微調，簡併能級亦可用緊湊錯位方式分行。
  * 無需切換模式即可在圖上連點兩個能級建立關聯線，也能直接點擊圖上連線進行編輯，並以對稱性完整配對與能量位置進行智慧自動連線。
  * 可調整不同軌域群／欄位之間的水平距離，適應單欄到多欄的版面需求。
  * 支援能量差雙箭頭、富文字上下標、自由文字、單位換算、JSON 專案保存及 SVG／高解析 PNG 匯出。

### 4. 📈 VASP 態密度分析與繪製儀 (VASP DOS Plotter)
...
### 5. ⚛️ 科研反應自由能差繪圖工具 (Reaction Free Energy Gap Plotter)

> 專為催化反應機理與理論計算設計的出版級反應自由能差（$\Delta G$ Profile / Grouped Bar Chart）繪圖工具。

* **工具入口**：[`energy-gap-plotter.html`](./energy-gap-plotter.html)
* **主要功能**：
  * **多體系分組柱狀圖對比**：支援多組催化體系／反應路徑並列呈現（如 $\text{HPO}_4^{2-}$ vs $\text{Pristine}$），清晰對比各步反應自由能差 $\Delta G$。
  * **$y=0$ 基準線與柱頂／底數值智慧排版**：高亮反應零基準線，正能差 ($\Delta G > 0$) 柱子向上延展並標註於柱頂、負能差 ($\Delta G < 0$) 柱子向下延展並標註於柱底。
  * **反應步階智慧輸入與轉換**：
    * 支援輸入 `A -> B`、`A --> B`、`\to` 自動轉換為標準學術箭頭 `→`。
    * **⚡ 一鍵生成反應步**：一鍵將中間體狀態序列轉換為相鄰反應步階（`State 0 → State 1`...）。
    * **🧮 自由能 $G \to \Delta G$ 能差計算機**：直接貼上各態總自由能 $G_i$，自動計算各步 $\Delta G_i = G_i - G_{i-1}$ 並生成步驟名稱與數值。
  * **X 軸步階排版與對齊控制**：
    * 提供多種標籤對齊模式：**置中對齊 (Center)**、**靠右／傾斜末端 (End)**、**靠左／起點 (Start)**。
    * 支援多角度旋轉（水平 0°、傾斜 30°/45°、垂直 90°）與垂直距離 (Y Offset) 微調，長分子式自動適應留白不重疊。
  * **化學式上下標與希臘字母引擎**：完美支援 `NO_3^-`、`HPO_4^{2-}`、`TS^*`、`\Delta G`、`\delta`、`\theta`、`\alpha` 等富文字上下標與學術符號。
  * **圖例自由拉動調控**：圖例支援滑鼠拖曳擺放位置，右下角控制手柄可直接拉動調整寬高，雙擊恢復最適尺寸 (Auto-fit)。
  * **出版級高畫質匯出**：支援內嵌樣式之向量 SVG、1x~6x (300~600 DPI) 高解析 PNG、Excel/TSV 剪貼簿貼上 (`Ctrl+V`) 與 JSON 專案儲存檔。
